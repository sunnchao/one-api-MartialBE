...
const OtherSetting = () => {
  ...
  const checkUpdate = async () => {
    try {
      if (!import.meta.env.VITE_REACT_APP_VERSION) {
        showError('无法获取当前版本号');
        return;
      }

      // 如果版本前缀是v开头的
      if (import.meta.env.VITE_REACT_APP_VERSION.startsWith('v')) {
        ...
        if (newVersion === import.meta.env.VITE_REACT_APP_VERSION) {
          ...
          <Grid container spacing={{ xs: 3, sm: 2, md: 4 }}>
            <Grid xs={12}>
              <Typography variant="h6" gutterBottom>
                {t('setting_index.otherSettings.generalSettings.currentVersion')}: {import.meta.env.VITE_REACT_APP_VERSION}
              </Typography>
          ...
          <Grid container spacing={{ xs: 3, sm: 2, md: 4 }}>
            ...
            <Grid xs={12}>
              <Alert severity="warning">
                移除 Chirou API 的版权标识必须首先获得授权，项目维护需要花费大量精力，如果本项目对你有意义，请主动支持本项目。
              </Alert>
            </Grid>
            <Grid xs={12}>
              <FormControl fullWidth>
