...
const EditModal = ({
  ...
  // 渲染缓存输入价格表单
  const renderCacheInputField = (formProps) => {
    const { errors = {}, touched = {}, handleBlur, handleChange: formikHandleChange, values = {} } = formProps || {};

    // 根据模式选择正确的处理函数和值
    const value = singleMode ? inputs.cache_input : values.cache_input;
    const onChange = singleMode ? handleChange : formikHandleChange;
    const errorState = singleMode ? !!errors.cache_input : Boolean(touched?.cache_input && errors?.cache_input);
    ...
    return (
      <FormControl fullWidth error={errorState} sx={{ ...theme.typography.otherInput }}>
        ...
        {(singleMode && errors.cache_input) || (!singleMode && touched?.cache_input && errors?.cache_input) ? (
