...
const claudeBaseUrl = "https://%saiplatform.googleapis.com/v1/projects/%s/locations/%s/publishers/anthropic/models/%s:%s"
...
func (p *VertexAIProvider) GetFullRequestURL(modelName string, other string) string {
	...
	// 判断是否是 DeepSeek 模型
	isDeepSeek := strings.Contains(modelName, "deepseek")

	if p.Region == "global" {
		if isClaude {
			return fmt.Sprintf(claudeBaseUrl, "", p.ProjectID, p.Region, modelName, other)
		}
	...
	// 对于 Claude 模型使用 anthropic publisher
	if isClaude {
		return fmt.Sprintf(claudeBaseUrl, p.Region+"-", p.ProjectID, p.Region, modelName, other)
	}
