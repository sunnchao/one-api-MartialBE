 ...
 * 端点类型映射配置
 */
const ENDPOINT_MAPPING: Record<number, string> = {
  8: 'OpenAI',
  14: 'Anthropic',
  25: 'Gemini',
  42: 'Gemini'
 ...
 */
export function getSupportedEndpoints(endPoints: number[]): string[] {
  ...
  const supportedTypes = [
    ...new Set(
      endPoints.map((id) => ENDPOINT_MAPPING[id]).filter(Boolean)
    )
  ];
 ...
 */
export function supportsEndpoint(endPoints: number[], endpointType: string): boolean {
 ...
 */
export function getEndpointColor(endpointType: string) {
