...
func addChannelKeysTable() *gormigrate.Migration {
  return &gormigrate.Migration{
    ID: "202506011001",
    Migrate: func(tx *gorm.DB) error {
      logger.SysLog("开始迁移channel_keys表 - 新版本")
      
      // 首先检查迁移表，看是否存在记录
      var migrationCount int64
      tx.Table("migrations").Where("id = ?", "202506011001").Count(&migrationCount)
      logger.SysLog(fmt.Sprintf("迁移记录检查: %d", migrationCount))
      
      // 检查表是否存在并且有数据
      var count int64
      err := tx.Model(&ChannelKey{}).Count(&count).Error
      if err != nil {
        logger.SysLog("检查channel_keys表失败: " + err.Error())
      } else {
        logger.SysLog(fmt.Sprintf("当前channel_keys表中有 %d 条记录", count))
      }
      ...
      err = tx.Where(keyCol + " != ''").Find(&channels).Error
      if err != nil {
        logger.SysLog("获取channels失败: " + err.Error())
        return err
      }
      
      logger.SysLog(fmt.Sprintf("找到 %d 个带key的channels", len(channels)))
      
      createdCount := 0
      for _, channel := range channels {
        // 检查是否已存在key
        var keyCount int64
        err = tx.Model(&ChannelKey{}).Where("channel_id = ?", channel.Id).Count(&keyCount).Error
        if err != nil {
          logger.SysLog("检查channel key失败: " + err.Error())
          continue
        }
        ...
        if keyCount == 0 {
          ...
          err = tx.Create(channelKey).Error
          if err != nil {
            logger.SysLog("为channel #" + strconv.Itoa(channel.Id) + "创建key失败: " + err.Error())
          } else {
            createdCount++
            logger.SysLog("为channel #" + strconv.Itoa(channel.Id) + "创建key成功")
          }
        } else {
          logger.SysLog("channel #" + strconv.Itoa(channel.Id) + "已有key记录，跳过")
        }
      }
      
      logger.SysLog(fmt.Sprintf("成功为 %d 个channel创建了key记录", createdCount))
      return nil
    },
    Rollback: func(tx *gorm.DB) error {
      return tx.Rollback().Error
