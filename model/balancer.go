...
type ChannelsChooser struct {
	...
	EndPoints  map[string]map[int]bool
}
...
func (cc *ChannelsChooser) GetGroupListModels(groupList []string) ([]string, error) {
	cc.RLock()
	defer cc.RUnlock()
	...
	for _, group := range groupList {
		_, ok := cc.Rule[group]
		if !ok {
			okList = append(okList, false)
		}
	}

	if len(okList) == len(groupList) {
		return nil, errors.New("group not found")
	}
	...
	for _, group := range groupList {
		...
		// 去重
		allModels = utils.RemoveDuplicate(allModels)
	}
...
func (cc *ChannelsChooser) GetModelsEndPoints() map[string]map[int]bool {
	cc.RLock()
	defer cc.RUnlock()
...
func (cc *ChannelsChooser) Load() {
	...
	newModelEndPoints := make(map[string]map[int]bool)
	...
	for _, channel := range channels {
		...
		for _, model := range models {
			if _, ok := newModelEndPoints[model]; !ok {
				newModelEndPoints[model] = make(map[int]bool)
			}
			newModelEndPoints[model][channel.Type] = true
		}
	...
	cc.EndPoints = newModelEndPoints
	cc.Unlock()
