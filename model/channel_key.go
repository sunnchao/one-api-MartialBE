...
const (
  ChannelKeyIsDeleted = 1
  ChannelKeyIsNotDeleted = 0
)
...
func GetChannelKeysByChannelId(channelId int) ([]*ChannelKey, error) {
  var keys []*ChannelKey
  err := DB.Where("channel_id = ? AND is_deleted = ?", channelId, ChannelKeyIsNotDeleted).Find(&keys).Error
  return keys, err
}
...
func GetNextAvailableKey(channelId int) (*ChannelKey, error) {
  var keys []*ChannelKey
  err := DB.Where("channel_id = ? AND is_deleted = ?", channelId, ChannelKeyIsNotDeleted).
...
func DeleteChannelKeys(channelId int) error {
  err := DB.Where("channel_id = ?", channelId).Update("is_deleted", ChannelKeyIsDeleted).Error
  return err
}
...
func EnableChannelKey(keyId int) error {
  err := DB.Model(&ChannelKey{Id: keyId}).Updates(map[string]interface{}{
    "status":      ChannelKeyStatusEnabled,
    "is_deleted":  ChannelKeyIsNotDeleted,
    "error_count": 0,
  }).Error
