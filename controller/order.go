...
func PaymentCallback(c *gin.Context) {
	...
	model.RecordLog(order.UserId, model.LogTypeTopup, fmt.Sprintf("在线充值成功，充值积分: %d，支付金额：%.2f %s", order.Quota, order.OrderAmount, order.OrderCurrency), utils.GetRequestIP(c))
